...
class SimulatorAimsLab(Simulator):
    ...
    def update_realtime_plot(self, position_traj_list: list, agents_position: list, targets_position: list, ax, obs_position_list=list(), obs_size_list=list()):
        """
        ...
        Inputs:
            ...
            ax: the axis object, generated by self.create_realtime_plot()
            obs_position_list: 2d list of obsatcle center positions in qualisys, [[x0,y0,z0], [x1,y1,z1], ...]
            obs_size_list: 2d list of obstacle size in meter. [[x0,y0], [x1,y1], ...],
                x0 is x-axis (height in cell map), y0 is y-axis (width in cell map)
        ...
        # plot obstacles
        self.plot_obs(ax, obs_position_list, obs_size_list)
    ...
    def plot_obs(self, ax, obs_position_list=list(), obs_size_list=list()):
        """
        ...
        if obs_position_list:
            for idx in range(len(obs_position_list)):
                pt_lbc = (obs_position_list[idx][1]+0.5*obs_size_list[idx][1],
                          obs_position_list[idx][0]-0.5*obs_size_list[idx][0])

                rect = patches.Rectangle(pt_lbc,
                                         width=-obs_size_list[idx][1],
                                         height=obs_size_list[idx][0],
                                         color="black", alpha=0.5)
                ax.add_artist(rect)
