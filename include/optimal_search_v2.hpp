#ifndef OPTIMAL_SEARCH_V2_HPP
#define OPTIMAL_SEARCH_V2_HPP
...
inline agent *initialize(const int& num_agent, const int& num_task, int agent_position[]) {
    ...
    head->agent_index = 1;
    head->num_task = num_task - num_agent + 1;
    ...
    for (int i = 1; i < num_task - num_agent + 1; i++) {
    ...
    for (int i = 1; i < num_agent; i++) {
        this_agent = this_agent->next_agent;
        task *temp = (task*)malloc(sizeof(task));
        temp->task_index = num_task - num_agent + i;
        this_agent->next_task = temp;
        temp->prev_task = NULL;
        temp->next_task = NULL;
        temp->next_in_list = NULL;
    }
...
inline void move_one_task_to_next_agent(agent *agent_ptr) {
    if (agent_ptr->next_task->next_task == NULL) {
        return;
    }
    task *task_to_move = agent_ptr->next_task->next_task;
    // only two task, set agent task to NULL
...
inline void move_all_task_back(agent *agent_ptr) {
    ...
    while (place_to_move->prev_agent->next_task->next_task == NULL) {
                                 ...
                                 int solution[])
{
    ...
    while (true) {
        ...
        while (agent_ptr->next_task->next_task == NULL) {
            agent_ptr = agent_ptr->prev_agent;
        }
